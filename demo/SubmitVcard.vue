<template>
    <div style="padding: 2rem 3rem; text-align: left;">
        <div class="field">
            <label class="label">VCard ID</label>
            <div class="control">
                <input :class="['input', ($v.form.username.$error) ? 'is-danger' : '']" type="text" placeholder="input VCard ID"
                       v-model="form.username">
            </div>
            <p v-if="$v.form.username.$error && $v.form.username.$invalid" class="help is-danger">This username is invalid. Min length 6 characters</p>
            <p v-else>
                <span v-show="errorMsg!=''" class="help is-danger">{{errorMsg}}</span>
                <span v-show="submitSuccess" class="help is-success">{{successMsg}}</span>
            </p>
            <footer class="card-footer">
                <div style="margin: 0 auto;">
                <button v-show="!checking" class="card-footer-item button_style" @click="checkVCardID" style="border-width:1px;">Submit Request</button>
                <img v-show="checking" src="data:image/gif;base64," alt="checking" />
            </div>
            </footer>
        </div>
    </div>
</template>

<script>
    import {validationMixin} from 'vuelidate'
    import { required, minLength } from 'vuelidate/lib/validators'
    import axios from 'axios';

    export default {
        props: ['clickedNext', 'currentStep'],
        mixins: [validationMixin],
        data() {
            return {
                form: {
                    username: ''
                },
                submitSuccess: false,
                checking: false,
                successMsg: '',
                reset_request_url: 'https://apis.d.vcard.com/id/v1/forgot_password/reset_request',
                errorMsg: ''
            }
        },
        validations: {
            form: {
                username: {
                    required,
                    minLength: minLength(6)
                }
            }
        },
        methods: {
            checkVCardID() {
                this.$v.form.$touch();
                this.errorMsg = '';
                this.successMsg = '';
                if(this.$v.form.username.$error) return;
                this.checking = true;
                if(this.submitSuccess && store.state['vcard_id']==this.form.username){
                    console.log('user found.');
                    this.successMsg = 'User found. Verification code has been sent to you phone number';
                    this.checking = false;
                    this.$emit('can-continue', {value: true});
                    return;
                }
                this.submitSuccess = false;
                var that = this;
                axios.post(this.reset_request_url, {
                    vcard_id: this.form.username
                  })
                  .then(function (response) {
                    console.log('response', response);
                    if(response.status == 200){
                        try {
                            var data = JSON.parse(response.data);
                            if(data.status==400){
                                that.errorMsg = data.message;
                            } else if(data.status==200){
                                that.submitSuccess = true;
                                that.successMsg = 'Account found. Verification code has been sent to you phone number';
                                that.$emit('can-continue', {value: true});
                            }
                            that.checking = false;
                        }
                        catch(err) {
                            console.log(err);
                            if (err instanceof SyntaxError
                                && (response.data.hasOwnProperty('errorMessage') ||  response.data.hasOwnProperty('success')) ) {
                                that.submitSuccess = true;
                                that.successMsg = 'Account found. Verification code has been sent to you phone number';
                                that.$emit('can-continue', {value: true});
                            } else {
                                that.errorMsg = "Error occured. Please Contact Administrator.";
                            }
                            that.checking = false;
                        }
                    }
                    if(that.submitSuccess) {
                        store.setState('vcard_id', that.form.username);
                        console.log('submitSuccess');
                        setTimeout(()=> {
                            that.$emit('force-click-next', {submitSuccess: that.submitSuccess});
                        }, 1000)
                    }

                  })
                  .catch(function (error) {
                    console.log('error', error);
                    that.checking = false;
                  })
            }
        },
        watch: {
            $v: {
                handler: function (val) {
                    this.errorMsg = '';
                    if(!val.$invalid) {
                        //this.$emit('can-continue', {value: true});
                    } else {
                        //this.$emit('can-continue', {value: false});
                        setTimeout(()=> {
                            this.$emit('change-next', {nextBtnValue: false});
                        }, 3000)
                    }
                },
                deep: true
            },

            clickedNext(val) {
                console.log(val);
                if(val === true) {
                    this.$v.form.$touch();
                }
            }
        },
        mounted() {
            if(!this.$v.$invalid && this.submitSuccess) {
                this.$emit('can-continue', {value: true});
            } else {
                this.$emit('can-continue', {value: false});
            }
        }
    }
</script>