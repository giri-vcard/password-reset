    ...
    <div class="card" style="margin: 3rem">
        <div class="card-content">
            ...
            <div class="field">
                ...
                <p class="help" :class="{'is-success': codeValid, 'is-danger': !codeValid }">{{msg}}</p>
            </div>

        </div>
        <footer class="card-footer" >
        <div style="margin: 0 auto;">
            <button v-show="!checking && !codeValid" class="card-footer-item button_style" @click="verifyCode" style="border-width:1px;">Verify Code</button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                codeValid: false,
                checking: false,
                verify_code_url: 'https://apis.d.vcard.com/id/v1/forgot_password/verify_code',
                msg: ''
            }
        },
        validations: {
        ...
        methods: {
            verifyCode() {
                this.msg = '';
                this.checking = true;
                if(this.codeValid && store.state['code']==this.form.code){
                ...
                var that = this;
                axios.post(this.verify_code_url, {
                    vcard_id: store.state.vcard_id,
                    code: this.form.code
                  })
                  .then(function (response) {
                    ...
                    if(response.status == 200){
                        try {
                            if(response.data.hasOwnProperty('reset_request_id')){
                                that.codeValid = true;
                                that.msg = 'Code is Valid';
                                that.$emit('can-continue', {value: true});
                                store.setState('code', that.form.code);
                                store.setState('request_id', response.data.reset_request_id);
                            } else{
                            ...
                            if (err instanceof SyntaxError) {
                                ...
                                that.msg = "Error occured. Please Contact Administrator.";
                            }
                        }
                    }
                    that.checking = false;
                    if(that.codeValid){
                        ...
                        that.$emit('force-click-next', {codeValid: that.codeValid});
                    }
                  })
