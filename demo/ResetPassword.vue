<template>
    <div style="padding: 2rem 3rem; text-align: left;">
        <div class="field">
            ...
            <div class="control">
                <input :class="['input', ($v.form.password.$error && $v.form.password.$invalid) ? 'is-danger' : '']" type="password" placeholder="Password"
                       v-model="form.password">
            </div>
            <p v-if="$v.form.password.$error && $v.form.password.$invalid" class="help is-danger">Password required with minimal 8 characters</p>
        </div>
        <div class="field">
            ...
            <p v-show="resetSuccess" class="help is-success">{{msg}}</p>
        </div>
        <footer class="card-footer">
        <div style="margin: 0 auto;">
            <button v-show="!checking && !resetSuccess"  class="card-footer-item button_style" @click="resetPassword" style="border-width:1px;">Reset Password</button>
    ...
    export default {
        ...
        data() {
            return {
                ...
                passwordMatch: false,
                resetSuccess: false,
                reset_url: 'https://apis.d.vcard.com/id/v1/forgot_password/reset',
                msg: ''
            }
        },
        validations: {
        ...
        methods: {
            ...
            console.log('this.resetSuccess', this.resetSuccess);
                if(this.passwordMatch && !this.resetSuccess) {
                    console.log('resetting..');
                    this.checking = true;
                    var that = this;
                    axios.post(this.reset_url, {
                      ...
                      .then(function (response) {
                        ...
                        if(response.status == 200 && response.data.success){
                            ...
                            that.msg = 'Reset Password Success';
                            that.$emit('can-continue', {value: true});
                            that.$emit('force-redirect', {success: true});
                        } else {
                            that.$emit('force-redirect', {success: false});
                        }
                        that.checking = false;
                      })
